...
const audioSprite = "https://s3.us-east-2.amazonaws.com/www.spyfall.app/sprite.mp3";
...
const PhoneBody = ({
  ...
  const [play] = useSound(audioSprite, {
    sprite: spriteMap,
    interrupt: true,
  });
  ...
  const handleLinkPress = (e) => {
    ...
    switch (destination) {
      case "instagram":
        if (confirmed(`Go to Jesse's Instagram?`)) redirect("https://www.instagram.com/jesselirola");
        break;
      case "website":
        if (confirmed(`Go to Jesse's portfolio site?`)) redirect("https://www.jesselirola.com");
        break;
      case "email":
        if (confirmed(`Open email to message Jesse?`)) redirect("mailto:jesse@jesselirola.com");
        break;
      default:
        return;
    }
